...
def updatePID():
    ...
    if abs(error) < (TOLERANCE_RADIANS) and abs(d) < TOLERANCE_RADIANS and not reached:
...
if __name__ == "__main__":
    rospy.init_node("turn_controller")
    ...
    cut_power_pub = rospy.Publisher("motors/cut_power", Float32, queue_size=10)

    rospy.Subscriber("/slam_out_pose", PoseStamped, onOdometryUpdate)
    rospy.Subscriber("/turn_command", Float32, onTarget)
    rospy.Subscriber("/motion_handler/quit", Float32, stop_holding)
